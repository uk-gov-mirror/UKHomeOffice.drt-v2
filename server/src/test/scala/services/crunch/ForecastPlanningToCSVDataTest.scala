...
class ForecastPlanningToCSVDataTest extends Specification {
  ...
  "Given a ForecastPeriod which in  cludes a period that spans a timezone change from BST to UTC " +
  ...
  "Given a ForecastPeriod which includes a period that spans a timezone change from UTC to BST " +
    ...
    val forecastPeriodDays: ForecastPeriod = forecastForPeriodStartingOnDay(2, SDate("2019-03-31T00:00:00Z"))
    val result = CSVData.forecastPeriodToCsv(forecastPeriodDays)

    val expected =
      s"""|,31/03 - available,31/03 - required,31/03 - difference,01/04 - available,01/04 - required,01/04 - difference
          |00:00,1,1,0,1,1,0
          |00:15,1,1,0,1,1,0
          |00:30,1,1,0,1,1,0
          |00:45,1,1,0,1,1,0
